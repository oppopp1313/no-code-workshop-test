...
class SmartHomeDashboard {
    ...
    getChartColors() {
        ...
        return {
            ...
            bgColor: isDark ? '#111111' : '#fafafa',
            primaryColor: isDark ? '#ffffff' : '#000000',
            secondaryColor: isDark ? '#cccccc' : '#333333'
        };
    }
        ...
        this.peakHourChart = Highcharts.chart('peakHourChart', {
            ...
            series: [{
                ...
                fillColor: {
                    linearGradient: { x1: 0, y1: 0, x2: 0, y2: 1 },
                    stops: [
                        [0, colors.primaryColor + '20'], // 12% opacity
                        [1, colors.primaryColor + '05']  // 2% opacity
                    ]
                }
            }],
            tooltip: {
    ...
    updateTotalUsage() {
        ...
        // 添加數字更新動畫
        const totalElement = document.getElementById('totalUsage');
        const lastPeriodElement = document.getElementById('lastPeriodUsage');
        ...
        // 數字動畫效果
        this.animateNumber(totalElement, parseFloat(totalElement.textContent.replace(/,/g, '')), data.total, 1000);
        this.animateNumber(lastPeriodElement, parseFloat(lastPeriodElement.textContent.replace(/,/g, '')), data.lastPeriod, 1000);
    ...
    toggleDevice(deviceId) {
        ...
        if (device && deviceId !== 'fridge') { // 冰箱不能關閉
            ...
            // 添加切換動畫效果
            deviceItem.classList.add('device-toggling');
            setTimeout(() => {
                deviceItem.classList.remove('device-toggling');
            }, 800);
    ...
    updateDeviceConsumption() {
        ...
        devices.forEach(([id, device], index) => {
            const percentage = (device.consumption / maxConsumption * 100).toFixed(0);
            
            const item = document.createElement('div');
            item.className = 'consumption-item animate-in';
            ...
            item.innerHTML = `
                ...
                <span class="consumption-value">${device.consumption.toFixed(1)} kWh</span>
            `;
